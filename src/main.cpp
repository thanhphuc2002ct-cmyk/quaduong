...
if (btnPressed)
    {
        ...
        if (digitalRead(BUTTON_PIN) == LOW)
        {
            ...
            while (digitalRead(BUTTON_PIN) == LOW)
            {
                ...
                extern void showButtonHoldProgress(unsigned long heldTime);
                showButtonHoldProgress(heldTime);
            ...
            if (isCalib)
            {
                Serial.println("[BUTTON] DA DE 5S -> BAT DAU AUTO CALIB MOTOR!");
                delay(1000); 
                
                extern unsigned long prev_time;
                prev_time = micros(); 
                
                extern void modeAutoCalibrate();
                modeAutoCalibrate();     
    ...
    if (irKey != 0 && currentMode == MODE_MAZE)
    {
        if (irKey == 'U' || irKey == 'D' || irKey == 'L' || irKey == 'R' || irKey == 'O' || irKey == '#')
        {
            remoteCmd = irKey;
        }
    }
